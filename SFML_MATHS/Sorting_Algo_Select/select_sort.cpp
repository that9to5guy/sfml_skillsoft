...
void selection_sort(std::vector<int> &myArray, sf::RenderWindow &window, std::vector<sf::RectangleShape> &rectangles, sf::Text &heading, sf::Sound &click_sound) {
    int temp = 0;
    ...
    for(int i = 0; i < nums - 1; i++) {
        ...
        if(minIndex != i) {
            std::swap(myArray[i], myArray[minIndex]);
        }
        ...
        rectangles[minIndex].setFillColor(sf::Color::Red);
        ...
        // Reset color of the current sorted element to cyan
        rectangles[i].setFillColor(sf::Color::Cyan);
        rectangles[minIndex].setFillColor(sf::Color::Cyan);
    }
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == Event::Closed) {
                ...
                if(!arr_sorted) {
                    selection_sort(myArray, window, rectangles, heading,click_sound);
                }
            }
        }
